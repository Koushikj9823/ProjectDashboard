...
export const addProjectTask = (project_task,history) => async dispatch =>{
    ...
    try{
        await axios.post("http://localhost:8090/api/board",project_task);
        history.push("/");
        dispatch({
            type:GET_ERRORS,
            payload:{}
...
export const getBacklog = () => async dispatch => {
    const res = await axios.get("http://localhost:8090/api/board/all")
    dispatch({
...
export const deleteProjectTask = pt_id => async dispatch => {
    if(window.confirm('Are you sure you want to delete Project Task ' + pt_id)
        ...
        await axios.delete("http://localhost:8090/api/board/"+pt_id);
        dispatch({
...
export const getProjectTask = (pt_id,history) => async dispatch => {
    try{
        const res = await axios.get("http://localhost:8090/api/board/"+pt_id);
        dispatch({
